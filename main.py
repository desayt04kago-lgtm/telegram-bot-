...
from data_base import users, is_new_user, write_to_logi, accept
import random

TOKEN = "8385956483:AAFvC00QiMggf6NsnYM_GcJlMqq7p165NSI"
USER_ID_MASTER = 2136204746
...
def accept_of_not(call):
    ...
    bot.answer_callback_query(call.id, "Ваше решение отправлено")
...
def to_accept_design_one(msg):
    ...
    if not(accept.read("user_id", msg.chat.id)):
...
def to_accept_design_two(msg):
    ...
    caption = f"Новый дизайн от: {name} {last_name} | ID: {user_id}"
